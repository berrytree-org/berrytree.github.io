---
title: "John Martin Berry Ga"
description: "John Martin Berry (1864–1937), son of Isaac Martin and Mary Ellen Anderson Berry, Georgia."
bookToc: true
families: ["David J Berry"]
periods: ["Civil War", "Reconstruction"]
tags: ["biography", "georgia", "orange-county-nc", "census"]
last_updated: 2026-04-07
qc_owner:
qc_webteam:
qc_genealogy:
content_source: "wp-export"
---

<div class="ancestor-page">

{{< breadcrumb trail="Home|/|Ancestors|/ancestors/|Family Lines|/family-lines/|John Martin Berry Ga" >}}

{{< ancestor-chain
  gen3="David J. Berry × Mary Blalock" loc3="Orange County, NC → Georgia" link3="/ancestors/david-j-berry-sr/"
  gen4="Robert Berry × Elizabeth Everitt" loc4="Milton County, Georgia" link4="/ancestors/robert-clark-berry-ga/"
  gen5="Isaac Martin Berry × Mary Ellen Anderson" loc5="Georgia"
  gen6="John Martin Berry" loc6="Georgia" link6="/ancestors/johnmartinberryga/"
/>}}

{{< ancestor-profile
  name="John Martin Berry"
  gen="6"
  born="July 6, 1864, Georgia"
  died="July 9, 1937"
  parents="Isaac Martin and Mary Ellen Anderson Berry"
>}}

## **The Story of John Martin Berry**

John Martin Berry born July 6th, 1864 in Georgia and died on July 9th, 1937. <!-- UNSOURCED -->

RESEARCH NEEDS TO BE DONE ON THIS COUSIN

---

<h2 class="ancestor-section">Children of John Martin Berry</h2>

<ol class="children-list">

<li class="child-entry">
  <div class="child-name">1. Annie Lee Berry</div>
</li>

<li class="child-entry">
  <div class="child-name">2. Joseph Marion Berry</div>
</li>

<li class="child-entry">
  <div class="child-name">3. Florrie Linda Berry</div>
</li>

<li class="child-entry">
  <div class="child-name">4. Joseph Mation Berry</div>
</li>

</ol>

To be Researched

---

<h2 class="ancestor-section">Census Records</h2>

{{< census-summary summary="To be researched." >}}

---

<div class="tree-link">
  <a href="/family-lines/david-j-berry-family-tree/">
    View the Full David J. Berry Family Line
    <span class="tree-link-sub">Descendants of David J. and Mary Blalock Berry</span>
  </a>
</div>

</div>
